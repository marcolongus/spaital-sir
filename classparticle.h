...
const double L = 55;                 //Lado del cuadrado
...
double p_dinit = 0.01;  //población inicial de refractarios dirigidos. 
...
//agregar otro generador de numero aleatorio, investigar.
...
particle evolution(vector<particle> &system, vector<int> &index, bool inter){
    ...
    for (int i=1; i<index.size(); i++){
        if (A.is_sane() && system[index[i]].is_infected()){
            if (dis_sir(gen_sir) < p_transmision) {
    ...
    //if ( A.is_refractary() ){ if(dis_sir(gen_sir)<p_recfractary){A.set_sane();} }
